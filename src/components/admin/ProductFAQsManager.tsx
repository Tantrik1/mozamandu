...
import { Plus, Edit, Trash2, GripVertical, HelpCircle, Search } from 'lucide-react';
    ...
    <Card>
      ...
      <CardContent>
        <p className="text-sm text-muted-foreground mb-4">
          Add product-specific Q&A that will appear as FAQ schema in Google search results.
        </p>
        ...
        {faqs.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground border rounded-lg border-dashed">
            ...
            <p className="text-sm">Add FAQs to help customers and improve SEO</p>
          </div>
        ) : (
          <div className="space-y-2">
            {faqs.map((faq) => (
              <div 
                key={faq.id} 
                className={`p-4 border rounded-lg ${!faq.is_active ? 'opacity-50 bg-muted/50' : 'bg-card'}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {!faq.is_active && (
                      <span className="text-xs text-yellow-600 mt-1 inline-block">Inactive</span>
                    )}
                  </div>
