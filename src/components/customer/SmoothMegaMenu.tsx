    ...
    <>
      <div className="relative">
        ...
        <div className="flex items-center space-x-6">
          {navbarItems.map((item) => {
            ...
            return (
              ...
              >
                <button className={`
                  flex items-center space-x-1 px-4 py-2 text-sm font-medium transition-all duration-200 ease-in-out rounded-md
                  ${isActive 
        ...
        {activeItem && isHovering && (
          <div
            ref={menuRef}
            className="absolute top-full left-1/2 transform -translate-x-1/2 w-screen max-w-6xl bg-white shadow-xl border border-gray-200 rounded-lg z-50 mt-2"
            onMouseEnter={handleMenuMouseEnter}
            onMouseLeave={handleMenuMouseLeave}
          >
            <div className="p-6">
              <div className="mb-4">
                <h3 className="text-xl font-bold text-gray-900 mb-2">{activeItem.category?.name}</h3>
                <div className="h-0.5 bg-gradient-to-r from-red-500 to-red-300 w-16"></div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {activeItem.category?.subcategories?.map((subcategory) => (
                  <Link
                    ...
                    className="group block p-4 rounded-lg border border-gray-100 hover:border-red-200 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 bg-white"
                    onClick={() => {
                      setActiveCategory(null);
                      setIsHovering(false);
                    }}
                    ...
                    <div className="flex items-start space-x-3">
                      <div className="flex-shrink-0">
                        {subcategory.image_url ? (
                          ...
                          <div className="w-12 h-12 bg-red-100 rounded-md flex items-center justify-center group-hover:bg-red-200 transition-colors duration-200">
