      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            ...
            <NavigationMenu>
              <NavigationMenuList>
                <NavigationMenuItem>
                  <NavigationMenuTrigger className={`transition-colors ${location.pathname.startsWith('/categories') || location.pathname.startsWith('/subcategories') ? 'text-red-600 font-medium' : 'text-gray-700 hover:text-red-600'}`}>
                    Categories
                  </NavigationMenuTrigger>
                  <NavigationMenuContent>
                    <div className="grid gap-3 p-6 w-[400px] lg:w-[500px] lg:grid-cols-2">
                      {categories.map(category => <div key={category.id} className="space-y-2">
                          <Link to={`/categories/${category.id}`} className="block text-sm font-medium text-gray-900 hover:text-red-600 transition-colors">
                            {category.name}
                          </Link>
                          <div className="space-y-1">
                            {category.subcategories.slice(0, 3).map(subcategory => <Link key={subcategory.id} to={`/subcategories/${subcategory.id}`} className="block text-xs text-gray-600 hover:text-red-600 transition-colors pl-3">
                                {subcategory.name} - ${subcategory.selling_price}
                              </Link>)}
                            {category.subcategories.length > 3 && <Link to={`/categories/${category.id}`} className="block text-xs text-blue-600 hover:text-blue-800 transition-colors pl-3">
            ...
            <Link to="/about" className={`transition-colors ${isActive('/about') ? 'text-red-600 font-medium' : 'text-gray-700 hover:text-red-600'}`}>
              About
            </Link>
        ...
        {mobileMenuOpen && <div className="lg:hidden border-t">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              <Link to="/categories" className="block px-3 py-2 text-base font-medium text-gray-700 hover:text-red-600" onClick={() => setMobileMenuOpen(false)}>
                Categories
              </Link>
              ...
              <Link to="/about" className="block px-3 py-2 text-base font-medium text-gray-700 hover:text-red-600" onClick={() => setMobileMenuOpen(false)}>
                About
              </Link>
