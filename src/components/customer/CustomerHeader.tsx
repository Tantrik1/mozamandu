...
export function CustomerHeader() {
  ...
  const fetchNavbarItems = async () => {
    const {
      data
    } = await supabase.from('navbar_items').select(`
        *,
        category:categories(
          id,
          name,
  ...
  const renderNavItem = (item: NavbarItem) => {
    switch (item.item_type) {
      case 'home':
        return <Link key={item.id} to="/" className="relative text-gray-700 hover:text-red-600 font-medium transition-all duration-300 py-2 px-1 after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-red-600 after:transition-all after:duration-300 hover:after:w-full">
            Home
          </Link>;
      case 'faq':
        return <Link key={item.id} to="/faq" className="relative text-gray-700 hover:text-red-600 font-medium transition-all duration-300 py-2 px-1 after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-red-600 after:transition-all after:duration-300 hover:after:w-full">
            FAQs
          </Link>;
      default:
        return null;
  ...
  const renderCategoryItem = (item: NavbarItem) => {
    ...
    return <div key={item.id} className="relative group" onMouseEnter={() => handleMegaMenuEnter(category.id)} onMouseLeave={handleMegaMenuLeave}>
        ...
        {openMegaMenu === category.id && <div className="fixed left-1/2 transform -translate-x-1/2 top-20 w-full max-w-4xl bg-white shadow-2xl border-t-4 border-red-500 z-50 transition-all duration-300" onMouseEnter={() => handleMegaMenuEnter(category.id)} onMouseLeave={handleMegaMenuLeave}>
            <div className="p-8 mt-2 ">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4">
                {category.subcategories?.map(subcategory => <Link key={subcategory.id} to={`/subcategories/${subcategory.id}`} className="group/item flex items-center space-x-4 p-4 rounded-xl hover:bg-gradient-to-r hover:from-red-50 hover:to-pink-50 transition-all duration-300 hover:shadow-lg hover:-translate-y-1" onClick={() => setOpenMegaMenu(null)}>
                    <div className="relative overflow-hidden rounded-lg">
                      {subcategory.image_url ? <img src={subcategory.image_url} alt={subcategory.name} className="w-16 h-16 object-cover group-hover/item:scale-110 transition-transform duration-300" /> : <div className="w-16 h-16 bg-gradient-to-br from-red-100 to-red-200 rounded-lg flex items-center justify-center group-hover/item:from-red-200 group-hover/item:to-red-300 transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              {/* Mobile Menu Button */}
              <Button variant="ghost" size="sm" className="lg:hidden p-2 hover:bg-red-50 transition-colors duration-300" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
          ...
          {isMobileMenuOpen && <>
              {/* Overlay */}
              <div className="fixed inset-0 bg-black bg-opacity-50 z-40 lg:hidden transition-opacity duration-300" onClick={closeMobileMenu} />
              ...
              <div className={`fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-white shadow-2xl z-50 lg:hidden transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-100">
                  ...
                  <Button variant="ghost" size="sm" onClick={closeMobileMenu} className="p-2 hover:bg-gray-100 rounded-full">
                    <X className="h-5 w-5" />
                  </Button>
                </div>
                ...
                <div className="overflow-y-auto h-full pb-20">
                  <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                      {otherItems.map(item => <div key={item.id} onClick={closeMobileMenu}>
                          {item.item_type === 'home' && <Link to="/" className="block text-gray-800 hover:text-red-600 font-medium py-3 px-4 rounded-lg hover:bg-red-50 transition-all duration-200">
                              Home
                            </Link>}
                          {item.item_type === 'faq' && <Link to="/faq" className="block text-gray-800 hover:text-red-600 font-medium py-3 px-4 rounded-lg hover:bg-red-50 transition-all duration-200">
                              FAQs
                            </Link>}
                        </div>)}
                    </div>
                    ...
                    return <div key={item.id} className="border border-gray-100 rounded-lg overflow-hidden">
                            {/* Category Button */}
                            <button onClick={() => toggleCategory(category.id)} className="w-full flex items-center justify-between p-4 text-left font-medium text-gray-800 hover:bg-gray-50 transition-colors duration-200">
                              ...
                              <div className="bg-gray-50 border-t border-gray-100">
                                {category.subcategories?.map(subcategory => <Link key={subcategory.id} to={`/subcategories/${subcategory.id}`} className="flex items-center space-x-3 p-4 hover:bg-white transition-colors duration-200" onClick={closeMobileMenu}>
                                    <div className="flex-shrink-0">
                                      {subcategory.image_url ? <img src={subcategory.image_url} alt={subcategory.name} className="w-10 h-10 object-cover rounded-lg" /> : <div className="w-10 h-10 bg-gradient-to-br from-red-100 to-red-200 rounded-lg flex items-center justify-center">
                    ...
                    {/* User Actions */}
                    {user && <div className="pt-6 border-t border-gray-100 space-y-4">
                        <button onClick={() => {
                    ...
                    {!user && <div className="pt-6 border-t border-gray-100">
                        <Link to="/auth" onClick={closeMobileMenu} className="block w-full text-center bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-medium py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-300">
                          Login
                        </Link>
                      </div>}
                  </div>
