...
const categories = [
  { icon: Package, label: 'Ordering & Shipping', color: 'bg-orange-50 hover:bg-orange-100 border-orange-200' },
  { icon: Ruler, label: 'Sizing & Fit', color: 'bg-teal-50 hover:bg-teal-100 border-teal-200' },
  { icon: Sparkles, label: 'Product Care', color: 'bg-amber-50 hover:bg-amber-100 border-amber-200' },
  { icon: RefreshCw, label: 'Returns & Exchanges', color: 'bg-pink-50 hover:bg-pink-100 border-pink-200' },
];
  ...
  return (
    <section className="py-12 md:py-16 lg:py-20 bg-gradient-to-b from-background to-muted/30">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Category Cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-10 md:mb-12 animate-fade-in" style={{ animationDelay: '0.1s' }}>
          {categories.map((cat, idx) => (
            <Link
              key={idx}
              to="/faq"
              className={`group flex flex-col items-center justify-center p-4 md:p-6 rounded-2xl border ${cat.color} transition-all duration-300 hover:shadow-md hover:-translate-y-1`}
            >
        ...
        <div className="animate-fade-in" style={{ animationDelay: '0.2s' }}>
          <div className="flex items-center gap-3 mb-6">
            <h3 className="text-lg md:text-xl font-semibold text-foreground">Popular Questions</h3>
