...
-- Create orders table for both guest and authenticated users
...
-- Create order_items table for inventory tracking
...
-- Create order_item_details table for display purposes
...
CREATE POLICY "Admins can manage payment screenshots" ON storage.objects
FOR ALL USING (bucket_id = 'payment-screenshots' AND auth.uid() IS NOT NULL AND public.is_admin());
