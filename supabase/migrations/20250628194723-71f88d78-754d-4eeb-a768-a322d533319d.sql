...
-- Create customer_order_item_details table
...
-- Enable RLS for customer_orders
...
-- Enable RLS for customer_order_items
...
CREATE POLICY "Admins can manage all customer order items" 
ON public.customer_order_items 
FOR ALL 
USING (public.is_admin());

-- Enable RLS for customer_order_item_details
...
CREATE POLICY "Admins can manage all customer order item details" 
ON public.customer_order_item_details 
FOR ALL 
USING (public.is_admin());
...
-- Transfer existing customer orders from orders table to customer_orders table
INSERT INTO customer_orders (
  user_id, order_number, customer_name, customer_email, contact_number, 
  whatsapp_number, delivery_address, subtotal, delivery_charge, 
  promocode_discount, total_amount, paid_amount, remaining_amount, 
  status, payment_method_id, payment_screenshot_url, promocode_used, 
  combo_applied, pricing_breakdown, delivery_location_id, created_at, updated_at
)
SELECT 
  user_id, order_number, customer_name, customer_email, contact_number,
  whatsapp_number, delivery_address, subtotal, delivery_charge,
...
-- Transfer order items for customer orders
INSERT INTO customer_order_items (order_id, product_id, color_variant_id, size_variant_id, quantity, created_at)
SELECT 
  co.id, oi.product_id, oi.color_variant_id, oi.size_variant_id, oi.quantity, oi.created_at
FROM order_items oi
JOIN orders o ON oi.order_id = o.id
JOIN customer_orders co ON o.order_number = co.order_number
WHERE o.user_id IS NOT NULL
AND o.user_id NOT IN (
  SELECT id FROM profiles WHERE role = 'admin'
);

-- Transfer order item details for customer orders
INSERT INTO customer_order_item_details (
  order_id, product_name, color_name, size_name, quantity, 
  unit_price, total_price, pricing_mode, pricing_details, created_at
)
SELECT 
  co.id, oid.product_name, oid.color_name, oid.size_name, oid.quantity,
  oid.unit_price, oid.total_price, oid.pricing_mode, oid.pricing_details, oid.created_at
...
-- Remove transferred orders from the original orders table
DELETE FROM orders 
WHERE user_id IS NOT NULL 
AND user_id NOT IN (
  SELECT id FROM profiles WHERE role = 'admin'
